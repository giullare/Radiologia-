...
existing.PianoId    = model.PianoId;
            ...
            if (piantina != null) existing.PiantinaZoneClassificateFile = piantina;

            var inailReg = await SalvaFile(fileInailReg, "inail");
            if (inailReg != null) existing.InailRicevutaRegistrazioneFile = inailReg;

            var inailCess = await SalvaFile(fileInailCess, "inail");
            if (inailCess != null) existing.InailRicevutaCessioneFile = inailCess;

            var strimsReg = await SalvaFile(fileStrimsReg, "strims");
            if (strimsReg != null) existing.StrimsRicevutaRegistrazioneFile = strimsReg;

            var strimsCess = await SalvaFile(fileStrimsCess, "strims");
            if (strimsCess != null) existing.StrimsRicevutaCessioneFile = strimsCess;
        ...
        public async Task<IActionResult> EliminaFile(int id, string campo)
        {
            ...
            // Mappa campo → percorso file e setter
            ...
            catch { /* ignora errori FS */ }

            // Azzera il campo nel DB
            switch (campo)
            {
                case "PiantinaZoneClassificateFile":
                    app.PiantinaZoneClassificateFile = null; break;
                case "StrimsRicevutaRegistrazioneFile":
                    app.StrimsRicevutaRegistrazioneFile = null; break;
                case "StrimsRicevutaCessioneFile":
                    app.StrimsRicevutaCessioneFile = null; break;
                case "InailRicevutaRegistrazioneFile":
                    app.InailRicevutaRegistrazioneFile = null; break;
                case "InailRicevutaCessioneFile":
                    app.InailRicevutaCessioneFile = null; break;
            }
